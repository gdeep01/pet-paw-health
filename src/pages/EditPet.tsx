...
import { Camera, Loader2, Dog, Cat, ArrowLeft } from 'lucide-react';
...
const EditPet = () => {
  ...
  const fetchPet = async () => {
    ...
    try {
      ...
      if (data.pet_photo_url) {
        setPhotoPreview(data.pet_photo_url);
      }
    } catch (error: any) {
      toast({
        title: 'Error',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: 'Pet Updated! 🎉',
        description: `${formData.pet_name}'s profile has been updated.`,
      });
      ...
      <form onSubmit={handleSubmit}>
        <Card className="border-border/50">
          <CardHeader>
            ...
            <CardDescription>
              {step === 1 && "Update your pet's basic details"}
              {step === 2 && "Update health-related information"}
              {step === 3 && "Update emergency and vet contacts"}
            </CardDescription>
          </CardHeader>
